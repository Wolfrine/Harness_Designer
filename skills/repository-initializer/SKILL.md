...
Do **not** create the subject dashboard or session shards during repository import itself. Bootstrap must first ask whether the user wants the dashboard. If declined, leave the capability dormant.
...
Keep `observability.dashboard` undecided until bootstrap asks the user, unless the user already gave a clear preference.
...
- optional dashboard state reflects only an explicit user choice;
...
Initialization is successful when Harness Designer has disappeared as a repository identity and what remains is unmistakably the user's subject repository with a minimal harness foundation ready for bootstrap, optional observability and future evolution.
